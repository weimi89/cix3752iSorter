...
每一步有檢查，卡住會停在那一步並印出原因，不會做到一半。**卡住時看它印的原因處理完，再跑一次 `to-new`**——它是「停舊 → 舊的不再自啟 → 開新」一整串，中途停下來就等於舊程式只是暫停、下次重開機還會自己回來。

**切換前不要先從選單把新程式點開來看。** 新程式一開就會登記「登入後自動啟動」並去連皮帶／分揀機；舊程式還在跑時兩邊會同時對裝置下指令，重開機後也會兩邊一起跳出來。`to-new` 會先把已開著的新程式關掉再走流程，但最乾淨的做法是切換前根本不要開它。要確認裝好了用 `sorter-switch status` 就夠。

下面是它做的事的手動版（一鍵版出錯時對照用）：
