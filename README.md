...
當「這件我們有收到」的證據；異常存證、包裹查詢都看得到。

- 本程式自己當 FTP 伺服器（預設 `0.0.0.0:2121`、帳密 `sorter`／`sorter`），只收 `STOR`，不給下載、刪檔、列目錄；PASV 與 PORT 兩種資料連線都接。
- 預設**存原圖**（`camera_ftp.max_edge_px = 0`；縮圖看不清楚）存 `data/images/YYYY-MM-DD/`。原圖一張約 620 KB、一天 9 千件約 5.7 GB，保留天數要跟硬碟算（200 GB 約 30 天）。填長邊就縮成證據圖（1600 約 110 KB），此時讀碼失敗的件另留原圖在 `orig/`。
- 照片對回包裹：檔名含窗口內某件的條碼就對那件；否則取窗口內（預設 5 秒）最早、有綁碼事件、還沒有照片的件。對不到的照片仍存檔並記事件。
- 保留天數獨立（`camera_ftp.retention_days`），與包裹資料分開；每小時清一次。
- 讀碼失敗的原因由系統從讀碼站回應判定（`db/abnormal_kind.rs`）：沒讀到碼、讀到鄰件的碼、讀到的不是單號、讀碼器沒回應；不做人工標記。
- 存放目錄可改（`camera_ftp.images_dir`，絕對路徑；空白 = `data/images`），要跟資料庫分開放到另一顆硬碟就填這裡；換目錄不搬舊圖。
- 畫面上照片與資料分開：照片按鈕／縮圖直接開 `ParcelImageViewer`（Viewer.js 可縮放、旋轉；標題帶條碼、格口、時間；`GET /api/parcel-images/{id}` 給 meta），包裹詳情只有資料、流程里程碑與白話時間軸。
